...
# Delete item
@app.route('/CategoryItem/delete/<int:categoryitem_id>',
           methods=['GET', 'POST'])
...
def gconnect():
    ...
    try:
        ...
        response = make_response(
            json.dump('Failed To authorization code.', 401))
    ...
    if result['issued_to'] != client_id:
        response = make_response(
            json.dump("Token's Client doesnt match given user id", 401))
    ...
    if stored_credential is not None and gplus_id == stored_gplus_id:
        response = make_response(
            json.dump("Current user is already connected", 200))
        response.headers['Content-Type'] = 'application/json'
        return response
...
@app.route('/itemjson.json')
def Itemjson():
    CategoryItems = session.query(CategoryItem).all()
    return jsonify(CategoryItem=[
        r.serialize for r in CategoryItems
    ])
